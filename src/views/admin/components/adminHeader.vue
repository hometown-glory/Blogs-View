...
const formRefs = ref<FormInstance | null>(null); // 定义表单引用
...
// 定义命令的类型
type CommandType = 'updatePassword' | 'logout';
...
watch(() => userStore.userInfo?.username, (newValue, oldValue) => {
  // 如果 newValue 是 undefined，则使用空字符串作为默认值
...
const dialogVisible = ref(false);  // 在这里定义 dialogVisible
const onSubmit = () => {
  // 先验证 form 表单字段
  formRefs.value?.validate((valid: boolean) => {
    ...
    // 调用修改用户密码接口
    updateAdminPassword({username: form.username, password: form.password}).then((res) => {
      console.log(res);
      // 判断是否成功
      if (res.success) {
