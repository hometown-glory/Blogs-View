    ...
    name: searchCategoryName.value
      ...
      .then((response) => {
        ...
        if (res.success === true) {
          ...
          // 直接使用 res.data 作为表格数据，因为它本身就是数组
...
const deleteCategorySubmit = (row: TableRow) => {
  console.log(row.id)
  showModel('是否确定要删除该分类？').then(() => {
    deleteCategory(row.id).then((res: AxiosResponse<AddCategoryResponse>) => {
      if (res.data.success) {
        showMessage('删除成功')
        // 重新请求分页接口，渲染数据
        getTableData()
...
<template>
  ...
  <!--    <el-form-item label="文章分类" prop="name" label-width="120px">-->
  <!--      &lt;!&ndash; 输入框组件 &ndash;&gt;-->
  <!--      <el-input size="large" v-model="form.name" placeholder="分类名称" clearable />-->
  <!--    </el-form-item>-->
  <!--  </el-form>-->
  <!--  <template #footer>-->
