...
import ScrollToTopButton from "@/views/fronted/components/ScrollToTopButton.vue";
import TocView from "@/views/fronted/components/TocView.vue";
...
// 获取文章详情
function refreshArticleDetail(articleId: number) {
...
watch(route, (newRoute) => {
  const newArticleId = parseInt(newRoute.params.articleId as string, 10)
...
onMounted(() => {
  ...
  if (articleContentRef.value) {
    observer.observe(articleContentRef.value as Node, config)
  }
})
          ...
          <article>
            ...
            <div class="text-gray-400 flex items-center mt-5 text-sm">
              ...
              <a @click="goCategoryArticleListPage(article.categoryId, article.categoryName)"
                 class="hover:underline">{{ article.categoryName }}</a>
            ...
            <!-- 正文 -->
            <div class="mt-5 article-content" v-viewer v-html="article.content"></div>
